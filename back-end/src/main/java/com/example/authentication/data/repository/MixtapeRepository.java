...
public interface MixtapeRepository extends JpaRepository<Mixtape, Integer> {
    ...
    @Query(value = "SELECT mixtape_id, user_id, genre_id, name, description, date, mixtape_pic_url FROM mixtapes ORDER BY date DESC", nativeQuery = true)
    List<Mixtape> findAll();
